...
{
    const int src_i420_y_size = width * height;
    const int src_i420_u_size = (width >> 1) * (height >> 1);
    ...
    const int dst_i420_y_size = dst_width * dst_height;
    const int dst_i420_u_size = (dst_width >> 1) * (dst_height >> 1);
    uint8_t *dst_i420_y_data = dst_i420_data;
...
int I420ToRGB565(const uint8_t *src_i420_data, int width, int height, uint8_t *dst_rgb565)
{
    ...
    const uint8_t *src_y = src_i420_data;
    const uint8_t *src_u = src_i420_data + src_y_size;
    const uint8_t *src_v = src_i420_data + src_y_size + src_u_size;
