...
Java_com_tang_ndkdemo_DeviceSurface_stringFromJNI(
        JNIEnv* env,
        jobject /* this */) {
...
Java_com_tang_ndkdemo_DeviceSurface_init(
        JNIEnv* env,
        jobject /* this */,
        jstring ip,
        jint port,
...
extern "C" JNIEXPORT void JNICALL
Java_com_tang_ndkdemo_DeviceSurface_close(
        JNIEnv* env,
        jobject /* this */) {
    if (NULL == g_data)
        return;
...
Java_com_tang_ndkdemo_DeviceSurface_append(
        JNIEnv* env,
        jobject /* this */,
        jint id,
        jobject surface) {
    ...
    g_surface = env->functions->NewGlobalRef(env, surface);
...
extern "C" JNIEXPORT void JNICALL
Java_com_tang_ndkdemo_DeviceSurface_remove(
        JNIEnv* env,
        jobject /* this */,
        jint id,
        jlong window) {
...
JNI_OnLoad(JavaVM *vm, void *res) {
    ...
    //av_register_all();
    g_read_ctx = (ReadContext *)malloc(sizeof(ReadContext));
...
void *read_thread(void *arg)
{
    ...
    ReadContext *readCtx = (ReadContext *)arg;

    do {
        /* open input file, and allocate utf8 context */
        if (avformat_open_input(&fmt_ctx, readCtx->url, NULL, NULL) < 0) {
        ...
        if (ret < 0) {
            LOGE("Could not find %s stream in input file '%s'\n",
                 av_get_media_type_string(AVMEDIA_TYPE_VIDEO), readCtx->url);
            break;
        }
        ...
        while (!readCtx->read_exit && av_read_frame(fmt_ctx, pkt) >= 0) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_tang_ndkdemo_DeviceSurface_openFfmpeg(
        JNIEnv* env,
        jobject /* this */,
        jstring url) {
    ...
    int len = strlen(utf8);
    memcpy(g_read_ctx->url, utf8, len);
    g_read_ctx->url[len] = '\0';
    g_read_ctx->read_exit = false;
    g_read_ctx->read_thread = 0;

#if 1
    int err;
    err = pthread_create(&g_read_ctx->read_thread, NULL, read_thread, g_read_ctx);
    if (err != 0) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_tang_ndkdemo_DeviceSurface_closeFfmpeg(
        JNIEnv* env,
        jobject /* this */) {
    int ret;

    g_read_ctx->read_exit = true;
    if (0 != g_read_ctx->read_thread) {
        ret = pthread_join(g_read_ctx->read_thread, 0);
        LOGV("stop read thread: %d\n", g_read_ctx->read_thread);
        g_read_ctx->read_thread = 0;
    }
}
